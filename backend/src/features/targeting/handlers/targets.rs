...
pub async fn list_targets(
    ...
    Query(_params): Query<TargetQueryParams>,
...
pub async fn create_target(
    ...
    let entity = state.ontology_svc.create_entity(entity_req)
        .await
        .map_err(|e| {
...
pub async fn update_target(
    ...
    let updated = state.ontology_svc.update_entity(&id, update_req)
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn get_target_timeline(
    ...
    timeline.push(serde_json::json!({
        "timestamp": entity.created_at.to_rfc3339(),
        "type": "TARGET_CREATED",
    ...
    if let Ok(entity_with_rels) = state.ontology_svc.get_entity_with_relationships(&id).await {
        ...
        for rel in &entity_with_rels.outgoing_relationships {
            timeline.push(serde_json::json!({
                "timestamp": rel.created_at.to_rfc3339(),
