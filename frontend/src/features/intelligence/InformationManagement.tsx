...
import { InterruptNotifier } from './InterruptNotifier';
import { useInterruptDetection } from './hooks/useInterruptDetection';
...
export function InformationManagement() {
    ...
    // Focus Mode & Interrupts
    const [focusMode, setFocusMode] = useState(false);

    // View Mode: 'grid' | 'timeline'
    const [viewMode, setViewMode] = useState<'grid' | 'timeline'>('grid');

    // Entity Selection for Drawer
    ...
    const rawRfis = entityGroups['RFI'] || [];
    const rawTasks = entityGroups['Task'] || [];

    // Interrupt Detection (uses raw lists to detect all incoming items)
    const { newUrgent, markAsChecked, dismissItem } = useInterruptDetection(rawRfis, rawTasks);
    ...
    const rfis = focusMode ? rawRfis.filter(isUrgent) : rawRfis;
    const tasks = focusMode ? rawTasks.filter(isUrgent) : rawTasks;
    ...
    return (
        <div className="flex flex-col h-full bg-[#020617] text-slate-200 overflow-hidden font-sans border-l border-slate-900 relative">
            ...
            <InterruptNotifier
                urgentItems={newUrgent}
                onDismiss={dismissItem}
                onAcknowledgeAll={markAsChecked}
                onView={(id: string) => {
                    dismissItem(id);
                    // In a real app, scrollTo or navigate
                    console.log('Viewing urgent item', id);
                }}
            />
                ...
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            ...
                            {focusMode && (
                                <div className="px-2 py-1 rounded bg-amber-500/10 border border-amber-500/20 text-amber-500 text-[10px] font-bold uppercase animate-pulse flex items-center gap-1">
                                    <AlertTriangle size={10} /> Focus Mode
                                </div>
                            )}
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            onClick={() => generateReport('IM Dashboard Status Report', entityGroups)}
                        ...
                        <div className="flex items-center bg-slate-900 rounded p-1 border border-slate-800">
                            <button
                                ...
                                className={cn("p-1.5 rounded transition-colors", viewMode === 'grid' ? "bg-slate-700 text-white shadow" : "text-slate-500 hover:text-slate-300")}
                                title="Grid View"
                            >
                                <Layers size={14} />
                            </button>
                            <button
                                ...
                                className={cn("p-1.5 rounded transition-colors", viewMode === 'timeline' ? "bg-slate-700 text-white shadow" : "text-slate-500 hover:text-slate-300")}
                                title="Timeline View"
                            >
                                <Clock size={14} />
                            </button>
                ...
                <div className="flex-1 overflow-y-auto">
                    {viewMode === 'timeline' ? (
                        <div className="bg-[#020617] min-h-full">
                            <TimelineView
                                entities={[...rfis, ...tasks, ...meetings, ...decisions]}
                                ...
                                <section className="bg-[#020617] p-6 space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {rfis.map((rfi) => (
                                            <div
                                                key={rfi.id}
                                                onClick={() => handleEntityClick(rfi.id)}
                                                className="p-4 rounded border bg-slate-900/30 border-slate-800 hover:border-purple-500/30 hover:bg-slate-900/60 transition-all flex gap-4 cursor-pointer group"
                                            >
                                                <div className={cn(
                                ...
                                <section className="bg-slate-950/30 p-6 space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {tasks.map((task) => (
                                            <div
                                                key={task.id}
                                                onClick={() => handleEntityClick(task.id)}
                                                className="relative pl-6 py-2 cursor-pointer hover:bg-slate-900/30 rounded transition-colors group"
                                            >
                                ...
                                <section className="bg-[#020617] p-6 space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {meetings.map((meeting) => (
                                            <div
                                                key={meeting.id}
                                                onClick={() => handleEntityClick(meeting.id)}
                                                className="p-4 rounded border bg-slate-900/30 border-slate-800 hover:border-purple-500/30 hover:bg-slate-900/60 transition-all cursor-pointer"
                                            >
                                ...
                                <section className="bg-slate-950/30 p-6 space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {tors.map((tor) => (
                                            <div
                                                key={tor.id}
                                                onClick={() => handleEntityClick(tor.id)}
                                                className="p-4 rounded border bg-slate-900/30 border-slate-800 hover:border-purple-500/30 hover:bg-slate-900/60 transition-all cursor-pointer"
                                            >
                                                <div className="flex justify-between items-start mb-2">
                                ...
                                <section className="bg-[#020617] p-6 space-y-4">
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {decisions.map((decision) => (
                                            <div
                                                key={decision.id}
                                                onClick={() => handleEntityClick(decision.id)}
                                                className="p-4 rounded border bg-slate-900/30 border-slate-800 hover:border-purple-500/30 hover:bg-slate-900/60 transition-all cursor-pointer"
                                            >
                                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        {persons.map((person) => (
                                            <div
                                                ...
                                                className="p-3 rounded border bg-slate-900/30 border-slate-800 hover:border-purple-500/30 hover:bg-slate-900/60 transition-all cursor-pointer"
                                            >
