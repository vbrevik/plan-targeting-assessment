...
export function BDAManagementView() {
    ...
    const getLatestReport = (targetId: string) => {
        return bdaReports
            .filter((r: BdaReport) => r.target_id === targetId)
            .sort((a: BdaReport, b: BdaReport) => new Date(b.assessment_date).getTime() - new Date(a.assessment_date).getTime())[0];
    };
    ...
    // Get unique target IDs from reports
    const targetIds = [...new Set(bdaReports.map(r => r.target_id))];
